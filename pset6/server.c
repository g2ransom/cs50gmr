...
char* indexes(const char* path)
{
    ...
    // create char arrays for both php and html paths
    char php[] = "index.php";
    char html[] = "index.html";
    
    // allocate enough memory for php and html paths
    char* php_path = malloc(strlen(path) + strlen(php) + 1);
    char* html_path = malloc(strlen(path) + strlen(html) + 1);
    
    // copy current path into php and html memory
    strcpy(php_path, path);
    strcpy(html_path, path);
    
    // add above char arrays to the appropriate paths
    strcat(php_path, php);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // if file is has nothing in it - return false
    if(file == NULL)
    {
    ...
    // counter variable that length will point to
    int counter = 0;
    
    if(buffer == NULL)
    {
        printf("The buffer is somehow empty.\n");
        return false;
    }
    
    // activate parameters for while loop
    *content = NULL;
    *length = 0;
    ...
    for(int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        ...
        counter++;
        buffer = realloc(buffer, (1 + counter) * sizeof(BYTE));
...
const char* lookup(const char* path)
{
    ...
    // create a variable that returns path type - ex: html, css, etc
    char* type = strrchr(path, '.');
    
    // go search for the path type and return given path
    if(strcasecmp(type,".css") == 0)
    {
        return "text/css";
    }
    
    else if(strcasecmp(type, ".html") == 0)
    {
        return "text/html";
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // create a copy of line that is dynamically allocated
    char* linecopy = malloc(strlen(line) + 1);
    strcpy(linecopy, line);
    linecopy[strlen(line)] = '\0';
    
    // separate all parts of line into tokens
    ...
    // if request target doesn't start with "/"
    if(req_target[0] != '/')
    {
        error(501);
        return false;
    ...
    // if request target has "" in string
    ...
    // split req_target into tokens
    char* abs_pathcopy = strtok(req_target, "?");
    
    char* query_copy = strtok(NULL, "\0");
    
    // abs_path may not have ? in it
    if(strchr(abs_pathcopy, '?') != NULL)
    {
        error(400);
        return false;
    }
    
    // abs_path must start with "/"
    if(abs_pathcopy[0] != '/')
    {
        error(400);
        return false;
    ...
    // if query isn't NULL then copy string to query
    if(query_copy != NULL)
    {
